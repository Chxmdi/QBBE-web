...
| Donation content | `/donate`, `components/donation-form.tsx`, `lib/payment-gates.ts` | No unsupported impact-per-dollar, tax, or restricted-fund claim; only a general designation is accepted; before an explicit QBBE launch flag the page does not render or submit donor fields; live checkout requires an explicit QBBE launch flag | Finance/legal/payment policy approval |
| Ally membership | `content/membership.ts`, `membershipPlan` schema, `lib/payment-gates.ts` | 2021 record structured, inactive, approval-gated; live checkout requires an explicit QBBE launch flag | Current benefits, price, Stripe setup |
| Registration | `/register`, student portal handoff, neutralized `/api/registrations` endpoint | New sensitive intake is not exposed or accepted before retention/consent review | Approved fields, purpose, RLS, retention, deletion |
| Global search | `lib/search.ts`, `/search` | Bilingual discovery index covers programs, route-specific pages, report archive records, and FAQs, grouped by content type | Sanity-backed search index and analytics once approved editorial content is connected |
...
| Build and regression checks | `npm run lint`, `npm test`, `npm run build` | Passed on 2026-08-17; 22 migration-focused tests, source-traceability guard, and 44 localized static pages | Browser/device/accessibility audit against deployed preview |

## Final source comparison status

The 70-record inventory is the authoritative comparison baseline currently available from QBBE-owned public sources. A final 2026-08-17 refresh of the 56-page public WordPress API index, five-post public index, two-product public index, verified 2020 Annual Report, and public student portal found no unexplained reviewed-content omission. The refresh also confirmed that the Daycare Program page had been modified on 2026-08-10 and that the Donate page had been modified on 2026-04-15; the former is now a distinct confirmation-gated program record and the latter’s classroom-item wish-list reference is preserved without publishing an unverified provider link. All records are classified `keep`, `rewrite`, `merge`, `archive`, `delete`, or `verify`; none were copied unchanged as `keep`. The differences intentionally retained in Website 2.0 are: blocked legacy pages, expired operational dates, historic prices, private forms, historic vacancies, inactive checkouts, unapproved translations, unverified current contact/leadership/partner information, unapproved documents, and absent current events/metrics/testimonials. Those differences are listed with owners in [CONTENT_VERIFICATION_REQUIRED.md](CONTENT_VERIFICATION_REQUIRED.md).
