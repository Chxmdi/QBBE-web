...
| Legacy inventory/disposition | `content/legacy-inventory.ts` | 66 discovered records carry a required disposition; final WordPress API page-and-post-index comparison has no unexplained omission | Complete old-site export and inaccessible route verification |
...
The 66-record inventory is the authoritative comparison baseline currently available from QBBE-owned public sources. A final 2026-08-17 comparison with the 56-page public WordPress API index and five-post public index found no unexplained indexed-content omission. All records are classified `keep`, `rewrite`, `merge`, `archive`, `delete`, or `verify`; none were copied unchanged as `keep`. The differences intentionally retained in Website 2.0 are: blocked legacy pages, expired operational dates, historic prices, private forms, historic vacancies, unapproved translations, unverified current contact/leadership/partner information, unapproved documents, and absent current events/metrics/testimonials. Those differences are listed with owners in [CONTENT_VERIFICATION_REQUIRED.md](CONTENT_VERIFICATION_REQUIRED.md).
